...
bot.onText(/\/start/, async (msg) => {
  ...
  bot.sendMessage(chatId, 'Welcome to my bot! Send a message, and I will see what the server has to say about it.');
});
...
bot.onText(/\/newsTech/, async (msg) => {
  const chatId = msg.chat.id;
  const response = await axios.get('http://localhost:3000/api/news/techcrunch');
